...
public class Main extends Canvas implements Runnable{
    ...
    public static int currentDayBorder = -1;
    ...
    public static boolean failedAuth = false;
    ...
    private static void setDate(){
        ...
        then = monday.with(next(FRIDAY));
        currentDate = dtf.format(monday);
        nextDate = dtf.format(then);

        DayOfWeek currentDayInDay = now.getDayOfWeek();
        if(currentDayInDay == MONDAY) {
            currentDayBorder = 0;
        } else if(currentDayInDay == TUESDAY) {
            currentDayBorder = 1;
        } else if (currentDayInDay == WEDNESDAY) {
            currentDayBorder = 2;
        } else if(currentDayInDay == THURSDAY) {
            currentDayBorder = 3;
        } else if(currentDayInDay == FRIDAY) {
            currentDayBorder = 4;
        }
    ...
    public Main() {
        ...
        this.addMouseWheelListener(new MouseWheelListener() {
            ...
            try {
                ...
                System.err.println("Add meg a bejelentkezési adataidat!");
            } finally {
                start();
                ...
                if(bearerCode == null || bearerCode.equals("")) {
    ...
    public void render() {
        ...
        if(isAuthed) {
            if (TIMETABLE != null) {
                if (!isSwitched) {
                    //mark current day differently
                    g.setColor(Color.CYAN);
                    g.fillRect(currentDayBorder*WIDTH/5, 0, WIDTH/5, HEIGHT);
